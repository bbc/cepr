...
class CreateProject extends React.Component {
	...
	handleProjectClick(e) {
		...
		if (this.state.renderProject) this.setState({ renderProject: false });
		else this.setState({ renderProject: true, currentWorkspace: e });
	}
	...
	render() {
		...
		const listItems = this.wsList.map(ws => (
			<div style={this.divStyle}>
				<div style={this.main}>
					...
					<div style={this.pagewrap}>created by Sports User on 22nd July, 2019</div>
				</div>
				<div>
				...
				<div>
					<b>
						{' '}
						<a href="#" onClick={this.handleProjectClick.bind(this, ws.workspaceName)}>
							{ws.projects ? ws.projects.length : 0} Projects{' '}
						</a>{' '}
					</b>{' '}
					&nbsp;&nbsp;&nbsp;
					<a href="#" onClick={this.handleClick.bind(this, ws.workspaceName)}>
						Create new project
					</a>{' '}
					&nbsp;&nbsp;&nbsp;
					<a href="#" onClick={this.handleClick.bind(this, 'create-project-ui')}>
						Add new member
					</a>
				</div>
					...
					{ws.projects ? (
						ws.projects.map(project =>
							this.state.renderProject && this.state.currentWorkspace == ws.workspaceName ? (
								<div style={this.divStyle}>{project.projectName}</div>
							) : (
								<div />
							)
						)
				...
				<div style={{ margin: '10px', width: 'auto', border: '1 solid' }}>
					{this.state.render != '' && this.state.currentWorkspace == ws.workspaceName ? (
						<Container state={this.state.render} wsname={ws.workspaceName} />
					) : null}
				</div>
			</div>
		));
		...
		return (
			<div style={this.divContainer} className="projects-form">
				...
				<div style={this.divStyle}>
					<b> {totalWorkSpaces} workspaces</b>
				</div>
				{listItems}
			</div>
		);
